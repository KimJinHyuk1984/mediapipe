<!-- ★ 공통 설계 문서: 템플릿 원본에서만 수정하고 sync-shared.sh 또는 sync-shared.ps1로 강의 저장소에 배포합니다.
     실제 공유 토큰의 단일 소스는 assets/css/tokens.css입니다. 강의별로 복제·변형하지 않습니다.
     출처: https://getdesign.md/mintlify/design-md
     원문 다운로드: https://getdesign.md/design-md/mintlify/DESIGN.md (2026-09-06)
     npx 실행이 PowerShell 실행 정책으로 실패해 위 페이지의 원문 다운로드 경로를 사용했습니다.
-->

# lecture-template 적용 규칙

아래 Mintlify 원문을 공통 설계 기준으로 보존합니다. 원문은 관찰한 UI의 분석이며,
이 프로젝트에서는 요청된 교실 가독성·오프라인 배포 조건을 다음과 같이 함께 적용합니다.
이 절은 강의마다 바꾸지 않습니다. 모든 값의 실제 구현은 `assets/css/tokens.css`에만 둡니다.

- 원문 색상: canvas `#ffffff`, canvas-dark / ink `#0a0a0a`, surface `#f7f7f7`,
  surface-soft `#fafafa`, hairline `#e5e5e5`, steel `#5a5a5c`, slate `#3a3a3c`, stone `#888888`.
- 원문 액센트: brand-green `#00d4a4`, brand-green-soft `#7cebcb`.
  민트는 강조 면과 활성 상태에 제한하며, 해당 면 위의 글자는 ink `#0a0a0a`를 사용합니다.
- 코드: surface-code `#1c1c1e`, on-dark `#ffffff`, on-dark-muted `#b3b3b3`를 두 테마에서 유지합니다.
- 원문 타이포: hero `72px / 600 / 1.05 / -2px`, heading-2 `36px / 600 / 1.2 / -0.5px`,
  heading-4 `22px / 600 / 1.3`, subtitle `18px / 400 / 1.5`를 적용합니다.
  모바일 hero는 원문의 `44px` 및 `36px`로 줄어듭니다.
- 교실 적용: 본문은 원문의 16px 대신 subtitle 크기인 **18px**, 보조 UI는 **17px**,
  코드는 원문의 14px 대신 **15px**를 사용합니다. 본문 최소 17px·코드 최소 15px를 지킵니다.
- 외부 폰트 없이 본문·제목은 `system-ui, "Pretendard", "Apple SD Gothic Neo", "Malgun Gothic", sans-serif`,
  코드는 `ui-monospace, "D2Coding", "Consolas", monospace`를 사용합니다. Inter·Geist Mono를 다운로드하지 않습니다.
- 원문 간격 `4/8/12/16/20/24/32/40px`를 `--space-1`부터 `--space-8`에 대응합니다.
  섹션 간격 `48/64/96px`, hero 간격 `120px`, 반경 `6/8/12px`, pill `9999px`를 유지합니다.
- 강의 본문 최대 너비는 요청된 **1080px**입니다. 모바일 내비게이션은 **768px 미만 가로 스크롤 칩**입니다.
- 프로젝트 확장: violet `#c4b5fd` / soft `#ede9fe`, amber `#ffd08a` / soft `#fff4df`를 제공합니다.
  라이트 액센트 글자는 mint `#006b53`, violet `#5b21b6`, amber·warn `#855300`으로 AA 대비를 확보합니다.
  원문 brand-warn `#c37d0d`는 밝은 배경의 작은 글자로 직접 쓰지 않습니다.
- 원문에 완성된 다크 팔레트가 없으므로 bg `#0a0a0a`, surface `#1c1c1e`, surface-2 `#141414`,
  border `#3a3a3c`, text `#ffffff`, muted `#b3b3b3`로 확장합니다. 다크 warn-soft는 `#332710`입니다.
- 코드 강조 배경은 `#303b37`; 문법 색은 keyword `#ceb5ff`, string `#7cebcb`,
  number `#ffd08a`, function `#a8d5ff`, comment `#b3b3b3`입니다. 모두 기본·강조 배경에서 AA를 만족해야 합니다.
- 제목 밑 장식용 밑줄, 카드 모서리 액센트 스트라이프, 의미 없는 그라데이션 배너는 사용하지 않습니다.
- 원문 권장 범위의 UI 전환 토큰은 **180ms**, 섹션 진입은 **450ms / 16px**입니다.
  테마 색은 전환 중에도 글자 대비를 유지하도록 즉시 바꿉니다.
  `prefers-reduced-motion: reduce`에서는 애니메이션·전환·부드러운 스크롤을 모두 끕니다.

## Mintlify 원문 (아래 원문 수치는 보존)
