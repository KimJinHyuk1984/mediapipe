...
(async () => {
  ...
  for (const id of ["step3", "tournament", "extend"]) {
  ...
  const out = fs.mkdtempSync(path.join(os.tmpdir(), "mediapipe-phase1b2-"));
  ...
  const ids = ["mission", "demo", "how", "learn", "setup", "step1", "opencv", "step2", "landmarks"];
  ...
  for (const name of ["requirements.txt", "test_install.py", "korean_text.py"]) {
    const [download] = await Promise.all([
      page.waitForEvent("download"),
      page.locator('#setup a[download][href="files/' + name + '"]').click()
    ]);
  ...
  console.log("PASS 9 sections / 3 TODO / images / all 14 clipboard contents");
  for (const width of [375, 1920]) {
    ...
    for (const theme of ["light", "dark"]) {
      ...
      for (const id of ["opencv", "landmarks"]) {
    ...
    for (const id of ids) {
      ...
      if (["setup", "step1", "step2", "landmarks"].includes(id)) {
